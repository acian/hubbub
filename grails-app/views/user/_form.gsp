...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'passwordHash', 'error')} ">
	...
	<g:textField name="passwordHash" maxlength="8" value="${userInstance?.passwordHash}"/>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'profile', 'error')} required">
	...
	<g:select id="profile" name="profile.id" from="${com.grailsinaction.Profile.list()}" optionKey="id" optionValue="displayString" required="" value="${userInstance?.profile?.id}" class="many-to-one"/>
