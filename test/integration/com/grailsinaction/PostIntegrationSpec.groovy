...
class PostIntegrationSpec extends Specification {
    ...
    def "Ensure a user can follow other users"() {
        ...
        def joe = new User(loginId: 'joe', password:'passwordHash').save()
        def jane = new User(loginId: 'jane', password:'passwordHash').save()
        def jill = new User(loginId: 'jill', password:'passwordHash').save()
